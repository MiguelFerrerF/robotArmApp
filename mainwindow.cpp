...
void MainWindow::connectVideoSignals() {
  ...
  connect(&handler, &VideoCaptureHandler::cameraInfoChanged, this,
          &MainWindow::onCameraInfoChanged);
...
void MainWindow::onCameraInfoChanged(const CameraInfo &info) {
  ...
  if (info.width > 0 && info.height > 0) {
    ui->lineEditResolution->setText(
        QString("%1 x %2 px").arg(info.width).arg(info.height));
  } else {
    ui->lineEditResolution->setText("Default");
  }
  ui->lineEditBrightness->setText(
      QString("%1 %").arg(static_cast<int>((info.brightness / 255.0) * 100)));
  ui->lineEditContrast->setText(
      QString("%1 %").arg(static_cast<int>((info.contrast / 255.0) * 100)));
  ui->lineEditSaturation->setText(
      QString("%1 %").arg(static_cast<int>((info.saturation / 255.0) * 100)));
  ui->lineEditSharpness->setText(
      QString("%1 %").arg(static_cast<int>((info.sharpness / 255.0) * 100)));
  if (!info.isFocusAuto)
    ui->lineEditFocus->setText(
        QString("%1 %").arg(static_cast<int>((info.focus / 255.0) * 100)));
  else
    ui->lineEditFocus->setText("Auto");
  if (!info.isExposureAuto)
    ui->lineEditExposure->setText(
        QString("%1 %").arg(static_cast<int>((info.exposure / 255.0) * 100)));
  else
